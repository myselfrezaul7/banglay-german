    ...
    return (
        <div className={`min-h-screen pb-24 bg-slate-50 dark:bg-slate-950 transition-opacity duration-500 overflow-hidden relative ${mounted ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <section className="relative pt-24 pb-12 text-center max-w-5xl mx-auto px-6 z-10">
                <div className="inline-flex items-center gap-2 mb-4 px-4 py-1.5 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 rounded-full text-sm font-bold tracking-widest uppercase border border-blue-200 dark:border-blue-800/50 shadow-sm animate-fadeInUp">
                    <BookOpen className="w-4 h-4" /> Word Library
                </div>
                <h1 className="text-4xl md:text-6xl font-extrabold mb-4 text-slate-900 dark:text-white font-poppins tracking-tight animate-fadeInUp" style={{ animationDelay: '0.1s' }}>
                    Master Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-500">Vocabulary.</span>
                </h1>
                <p className="text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed animate-fadeInUp" style={{ animationDelay: '0.2s' }}>
            ...
            {/* Floating Stats Bar */}
            <section className="max-w-4xl mx-auto px-6 relative z-20 -mt-2 animate-fadeInUp" style={{ animationDelay: '0.3s' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {levels.map((level, i) => (
                        <Link
                            ...
                            className="group relative block overflow-hidden rounded-[2rem] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 hover:border-transparent transition-all duration-500 hover:-translate-y-2 shadow-xl shadow-slate-200/20 dark:shadow-none animate-fadeInUp"
                        ...
                        >
                            ...
                            <div className="relative z-10 flex flex-col h-full">
                                <div className="flex justify-between items-start mb-6">
                                    <div className={`w-16 h-16 rounded-[1.2rem] bg-gradient-to-br ${level.colorFrom} ${level.colorTo} flex items-center justify-center text-white text-2xl font-black shadow-lg transform group-hover:scale-110 transition-transform duration-500`}>
