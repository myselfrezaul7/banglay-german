            ...
            <section className="py-16 bg-[var(--bg-secondary)]">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    <h1 className="text-4xl md:text-5xl font-bold mb-4">
                        <span className="gradient-text">Sentences</span>
                    </h1>
                    <p className="text-xl text-[var(--text-secondary)] mb-2">
                        Learn German through practical sentences
                    </p>
                    <p className="text-lg font-bengali text-[var(--text-muted)]">
                        ব্যবহারিক বাক্য দিয়ে জার্মান শিখুন
                    </p>
                </div>
            </section>

            {/* Filter */}
            <section className="py-6 border-b border-[var(--border-color)] sticky top-20 bg-[var(--bg-primary)] z-40">
                        ...
                        >
                            ...
                            <div className="p-6">
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1">
                                        ...
                                        {/* German */}
                                        <h3 className="text-xl font-semibold mb-3">{sentence.german}</h3>

                                        {/* Translations */}
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-2">
                                                <span className="text-xs px-2 py-0.5 rounded bg-[var(--info)] text-white">EN</span>
                                                <span className="text-[var(--text-secondary)]">{sentence.english}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <span className="text-xs px-2 py-0.5 rounded bg-[var(--accent-gold)] text-black font-bengali">বাং</span>
                                                <span className="text-[var(--text-secondary)] font-bengali">{sentence.bangla}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <button
                                            ...
                                            className="p-3 rounded-lg bg-[var(--bg-tertiary)] hover:bg-[var(--primary)] transition-all"
                                            title="Listen"
                                        >
                            ...
                            {expandedId === sentence.id && sentence.wordBreakdown && (
                                <div className="border-t border-[var(--border-color)] bg-[var(--bg-secondary)] p-6">
                                    <h4 className="text-sm font-medium text-[var(--text-muted)] mb-4">Word Breakdown</h4>
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                        {sentence.wordBreakdown.map((word, index) => (
                                            ...
                                            >
                                                <div className="font-semibold text-[var(--primary)]">{word.german}</div>
                                                <div className="text-sm text-[var(--text-secondary)]">{word.english}</div>
                                                <div className="text-sm text-[var(--text-muted)] font-bengali">{word.bangla}</div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
